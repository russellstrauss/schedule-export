...
describe('toGoogleEvent', () => {
  it('should transform a basic event correctly', () => {
    ...
    expect(result.summary).toBe('7:30am ERYKAH BADU - SH [Rhino]'); // 30 min before 8am
  ...
  it('should not use Rhino UNCONFIRMED title rules for non-rhino sources', () => {
    ...
    expect(result.summary).toBe('7:30am SECOND PORTAL GIG - SH [Crew 1]');
  ...
  it('should handle "called" status with UNCONFIRMED prefix', () => {
    ...
    expect(result.summary).toBe('UNCONFIRMED => 2025 T-MOBILE SEC CHAMPIONSHIP - SH [Rhino]');
    expect(result.status).toBe('tentative');
  });
...
describe('logAndMapEvents future-only filtering', () => {
  ...
  it('uses the venue when an IATSE show title is unavailable', () => {
    ...
    expect(mapped.map((event) => event.summary)).toEqual([
      '8:30am Lakewood Amphitheatre - SH [IATSE]',
      '8:30am State Farm Arena - SH [IATSE]'
    ]);
  });
});
